...
namespace MarketClient
{
    public class SimpleHTTPClient : ISimpleHTTPClient 
    {
        ...
        public T2 SendPostRequest<T1,T2>(string url, string user, string token, string nonce, string privateKey, T1 item) where T2 : class 
        {
            var response = SendPostRequest(url, user, token, nonce, privateKey , item);
